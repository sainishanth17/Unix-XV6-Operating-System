...
_prngtest:     file format elf32-i386
...
int main(void) {
   ...
   b:	56                   	push   %esi
    ...
    int min = 0x7FFFFFFF; // Maximum positive integer
    int max = 0x80000000; // Minimum negative integer
   c:	be 00 00 00 80       	mov    $0x80000000,%esi
#include "stat.h"
#include "user.h"
...
int main(void) {
  ...
  15:	81 ec b0 01 00 00    	sub    $0x1b0,%esp
    int i;
    int min = 0x7FFFFFFF; // Maximum positive integer
    int max = 0x80000000; // Minimum negative integer
    int sum = 0;
    int count[100] = {0};
  1b:	8d 5c 24 20          	lea    0x20(%esp),%ebx
...
int main(void) {
    int i;
    int min = 0x7FFFFFFF; // Maximum positive integer
  23:	bf ff ff ff 7f       	mov    $0x7fffffff,%edi
    int max = 0x80000000; // Minimum negative integer
    int sum = 0;
    int count[100] = {0};
  28:	c7 44 24 18 e8 03 00 	movl   $0x3e8,0x18(%esp)
  2f:	00 
...
int main(void) {
    ...
    int max = 0x80000000; // Minimum negative integer
    int sum = 0;
  30:	c7 44 24 1c 00 00 00 	movl   $0x0,0x1c(%esp)
  37:	00 
    int count[100] = {0};
  ...
  38:	e8 bd 03 00 00       	call   3fa <chr>
  3d:	89 c1                	mov    %eax,%ecx
        if (num < min) min = num;
        if (num > max) max = num;
        sum += num;
        int bin = num % 100;
  3f:	b8 1f 85 eb 51       	mov    $0x51eb851f,%eax
    int sum = 0;
    int count[100] = {0};
     ...
     for (i = 0; i < 1000; i++) {
        int num = (chr() & 0x7FFFFFFF); // Ensure the number is non-negative
  44:	81 e1 ff ff ff 7f    	and    $0x7fffffff,%ecx
  4a:	39 cf                	cmp    %ecx,%edi
  4c:	0f 4f f9             	cmovg  %ecx,%edi
  4f:	39 ce                	cmp    %ecx,%esi
  51:	0f 4c f1             	cmovl  %ecx,%esi
        if (num < min) min = num;
        if (num > max) max = num;
        sum += num;
        int bin = num % 100;
  54:	f7 e9                	imul   %ecx
     ...
     for (i = 0; i < 1000; i++) {
        ...
        if (num > max) max = num;
        sum += num;
  56:	01 4c 24 1c          	add    %ecx,0x1c(%esp)
        int bin = num % 100;
  5a:	c1 ea 05             	shr    $0x5,%edx
  5d:	6b d2 64             	imul   $0x64,%edx,%edx
  60:	29 d1                	sub    %edx,%ecx
        count[bin]++;
  62:	83 44 8c 20 01       	addl   $0x1,0x20(%esp,%ecx,4)
    int max = 0x80000000; // Minimum negative integer
    int sum = 0;
    int count[100] = {0};
     ...
     for (i = 0; i < 1000; i++) {
  67:	83 6c 24 18 01       	subl   $0x1,0x18(%esp)
  6c:	75 ca                	jne    38 <main+0x38>
    ...
    printf(1, "Min: %d\n", min);
  6e:	89 7c 24 08          	mov    %edi,0x8(%esp)
  72:	c7 44 24 04 16 08 00 	movl   $0x816,0x4(%esp)
  79:	00 
  7a:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  81:	e8 2a 04 00 00       	call   4b0 <printf>
    printf(1, "Max: %d\n", max);
  86:	89 74 24 08          	mov    %esi,0x8(%esp)
    printf(1, "Mean: %d\n", sum / 1000);
  8a:	31 f6                	xor    %esi,%esi
    ...
    printf(1, "Min: %d\n", min);
    printf(1, "Max: %d\n", max);
  8c:	c7 44 24 04 1f 08 00 	movl   $0x81f,0x4(%esp)
  93:	00 
  94:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  9b:	e8 10 04 00 00       	call   4b0 <printf>
    printf(1, "Mean: %d\n", sum / 1000);
  a0:	8b 44 24 1c          	mov    0x1c(%esp),%eax
  a4:	ba d3 4d 62 10       	mov    $0x10624dd3,%edx
  a9:	c7 44 24 04 28 08 00 	movl   $0x828,0x4(%esp)
  b0:	00 
  b1:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  b8:	f7 ea                	imul   %edx
  ba:	8b 44 24 1c          	mov    0x1c(%esp),%eax
  be:	c1 fa 06             	sar    $0x6,%edx
  c1:	c1 f8 1f             	sar    $0x1f,%eax
  c4:	29 c2                	sub    %eax,%edx
  c6:	89 54 24 08          	mov    %edx,0x8(%esp)
  ca:	e8 e1 03 00 00       	call   4b0 <printf>

    // Print histogram with range
    for (i = 0; i < 100; i++) {
  cf:	31 c0                	xor    %eax,%eax
  d1:	eb 07                	jmp    da <main+0xda>
  d3:	90                   	nop
  d4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  d8:	89 f8                	mov    %edi,%eax
        int startRange = i * (0x7FFFFFFF / 100);
        int endRange = (i + 1) * (0x7FFFFFFF / 100) - 1;
  da:	8d 78 01             	lea    0x1(%eax),%edi
        printf(1, "%d th bin (%d - %d): %d\n", i, startRange, endRange, count[i]);
  dd:	8b 54 bb fc          	mov    -0x4(%ebx,%edi,4),%edx
  e1:	89 74 24 0c          	mov    %esi,0xc(%esp)
  e5:	89 44 24 08          	mov    %eax,0x8(%esp)
  e9:	c7 44 24 04 32 08 00 	movl   $0x832,0x4(%esp)
  f0:	00 
  f1:	89 54 24 14          	mov    %edx,0x14(%esp)
  f5:	8d 96 13 ae 47 01    	lea    0x147ae13(%esi),%edx
  fb:	81 c6 14 ae 47 01    	add    $0x147ae14,%esi
 ...
 10c:	e8 9f 03 00 00       	call   4b0 <printf>
    ...
    // Print histogram with range
    for (i = 0; i < 100; i++) {
 111:	83 ff 64             	cmp    $0x64,%edi
 114:	75 c2                	jne    d8 <main+0xd8>
        int startRange = i * (0x7FFFFFFF / 100);
        int endRange = (i + 1) * (0x7FFFFFFF / 100) - 1;
        printf(1, "%d th bin (%d - %d): %d\n", i, startRange, endRange, count[i]);
    }
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 449:	0f b6 92 52 08 00 00 	movzbl 0x852(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5df:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 5e1:	b8 4b 08 00 00       	mov    $0x84b,%eax
...
malloc(uint nbytes)
{
 ...
 739:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 73c:	8b 1d d0 0a 00 00    	mov    0xad0,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 779:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 77b:	3b 15 d0 0a 00 00    	cmp    0xad0,%edx
...
morecore(uint nu)
{
 ...
 7ad:	e8 ee fe ff ff       	call   6a0 <free>
  return freep;
 7b2:	8b 15 d0 0a 00 00    	mov    0xad0,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 7d4:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 7d7:	89 15 d0 0a 00 00    	mov    %edx,0xad0
      return (void*)(p + 1);
 ...
 7ec:	eb e9                	jmp    7d7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7ee:	c7 05 d0 0a 00 00 d4 	movl   $0xad4,0xad0
 7f5:	0a 00 00 
    base.s.size = 0;
 7f8:	ba d4 0a 00 00       	mov    $0xad4,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7fd:	c7 05 d4 0a 00 00 d4 	movl   $0xad4,0xad4
 804:	0a 00 00 
    base.s.size = 0;
 807:	c7 05 d8 0a 00 00 00 	movl   $0x0,0xad8
 80e:	00 00 00 
